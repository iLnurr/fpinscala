...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @tailrec
    def go(d: Int, prev2: Int, prev: Int): Int = {
      if (d == 0) prev2
      else go(d - 1, prev, prev + prev2)
    }
    go(n, 0, 1)
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @tailrec
    def go(index: Int, isSorted: Boolean = true): Boolean = {
      if (index >= as.length) isSorted
      else {
        if (!isSorted) false
        else go(index + 1, gt(as(index), as(index + 1)))
      }
    }
    go(0)
  }
